	...
	nzwa.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `flw, talvez vc volte um dia🐊 @${num.split('@')[0]}😴`
				let buff = await getBuffer(ppimg)
				nzwa.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	nzwa.on('chat-update', async (mek) => {
		try {
			...
			const isCmd = body.startsWith(prefix)
                        ...
                        mess = {
                                wait: 'Em processo, tenha calma⚡',
                                succsess: '✔️ ai irmão ✔️',
			...
			const levelRole = getLevelingLevel(sender, _level)
                ...
                if (isGroup) {
					...
					setTimeout( () => {
						nzwa.updatePresence(from, Presence.composing)
						reply("5 segundos flw kkkkkkkkk")
					}, 0)
                                }
                        } catch {
                                console.error(err)
		        ...
		        if (!isGroup) return
		        if (!isAntiLink) return
		        if (isGroupAdmins) return reply('porque você é um administrador de grupo, o bot nao ira te remover 🐊')
		        nzwa.updatePresence(from, Presence.composing)
		        if (messagesC.includes("#izinadmin")) return reply("#izinadmin recebido")
		        var kic = `${sender.split("@")[0]}@s.whatsapp.net`
		        reply(`Link de grupo detectado ${sender.split("@")[0]} Você será expulso do grupo Em 5 segundos se ferrou lkkkkkkk`)
		        setTimeout( () => {
			        nzwa.groupRemove(from, [kic]).catch((e)=>{reply(`*ERR:* ${e}`)})
		        }, 5000)
		        setTimeout( () => {
		...
		setTimeout( () => {
		    nzwa.updatePresence(from, Presence.composing)
			reply("Nesse grupo, não gostamos de racismos, que isso sirva de exemplo 🚶flw flw")
		}, 4000)
		setTimeout( () => {
		    nzwa.updatePresence(from, Presence.composing)
			reply("2 segundos")
		...
		nzwa.sendMessage(from, bahasa(prefix, sender), text, {quoted: mek})
               ...
               break
				...
				case 'rebaixar':
					...
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Virou Membro Comum🤡se fudeu kskskksksk')
			...
			tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
				...
				case 'marcar':
					...
					mentions('Mencionei Tudo foudassi \©'+teks+'🐊', members_id, true)
					break
                case 'tagall2':
                case 'marcar2':
				nzwa.updatePresence(from, Presence.composing) 
